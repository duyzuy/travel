...
const FlightType: React.FC<{ oneStop?: boolean; durationTime?: number }> = ({
  oneStop = false,
  durationTime = 10000000,
}) => {
  ...
  return (
    <span className="trip-from-info text-center">
      ...
      <span className="pointers w-40 relative h-2 my-1 flex items-center mx-5">
        ...
        {(oneStop && (
          <>
            <span className="poin-start w-2 h-2 border border-slate-300 mx-1 block rounded-full right-0"></span>
            <span className="line  flex-1 w-full border-t border-slate-200 block"></span>
          </>
        )) || <></>}
      ...
      <span className="code text-sm text-slate-500">
        {(oneStop && "1 điểm dừng") || "Bay thẳng"}
      </span>
    </span>
  );
};
