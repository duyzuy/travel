  ...
  return (
    <span className="trip-from-info text-center">
      ...
      <span className="pointers w-40 relative h-2 my-1 flex items-center mx-5">
        ...
        {(transitTickets &&
          transitTickets.map((_transit, ind) => (
            <React.Fragment key={ind}>
              <span className="w-6 relative block text-center">
                <span className="poin-start w-2 h-2 border border-slate-300 rounded-full mx-auto flex items-center justify-center">
                  <span className="w-1 h-1 block bg-slate-600 rounded-full"></span>
                </span>
                <span className="text-xs block text-gray-600 absolute left-0 right-0 text-center mt-1">
                  {_transit.arrivalAirport}
                </span>
              </span>
              <span className="line flex-1 w-full border-t border-slate-200 block"></span>
            </React.Fragment>
