...
import { isShowLoginModalVar } from "@/cache/vars/profile";
...
const ContactInformation: React.FC<IContactInformation> = ({
  ...
  const { onShowModal } = useModal(isShowLoginModalVar);
  const onSelectPassengerTitle = (value: string) => {
    onAddContact({
      ...contactInformation,
      title: value as PASSENGER_TITLE,
    });
  };
          ...
          <div className="flex items-center bg-emerald-50 px-3 py-2 rounded-lg">
            ...
            <div className="remind-text">
              <p>
                ...
                để lưu lịch sử chuyến đi của bạn và tiến hành tích điểm đổi quà
              </p>
            </div>
          </div>
        </div>
